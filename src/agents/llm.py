...
def llm(  # noqa: PLR0911 — explicit returns map 1:1 to documented routing branches
    ...
    *,
    ...
    if escalate and settings.anthropic_api_key:
        return _build_claude(settings, temperature=temperature)
...
def _build_ollama(base_url: str, model: str, *, temperature: float) -> BaseChatModel:
    """Build a ChatOllama client. base_url should NOT contain /v1 suffix.

    Defensive: if a legacy /v1 suffix leaks in (e.g. someone sets
    ollama_p40_url to the old OpenAI-compat URL), strip it. ChatOllama uses
    Ollama's native /api routes, not the /v1 OpenAI shim.
    """
    base_url = base_url.removesuffix("/v1").rstrip("/")
    return ChatOllama(model=model, base_url=base_url, temperature=temperature)


def _build_claude(settings: Settings, *, temperature: float) -> BaseChatModel:
